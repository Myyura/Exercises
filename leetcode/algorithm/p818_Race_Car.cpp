/**
 * Myyura
 */

/*
 * @lc app=leetcode id=818 lang=cpp
 *
 * [818] Race Car
 *
 * https://leetcode.com/problems/race-car/description/
 *
 * algorithms
 * Hard (37.35%)
 * Likes:    393
 * Dislikes: 47
 * Total Accepted:    15.4K
 * Total Submissions: 41.2K
 * Testcase Example:  '3'
 *
 * Your car starts at position 0 and speed +1 on an infinite number line.
 * (Your car can go into negative positions.)
 * 
 * Your car drives automatically according to a sequence of instructions A
 * (accelerate) and R (reverse).
 * 
 * When you get an instruction "A", your car does the following: position +=
 * speed, speed *= 2.
 * 
 * When you get an instruction "R", your car does the following: if your speed
 * is positive then speed = -1 , otherwise speed = 1.  (Your position stays the
 * same.)
 * 
 * For example, after commands "AAR", your car goes to positions 0->1->3->3,
 * and your speed goes to 1->2->4->-1.
 * 
 * Now for some target position, say the length of the shortest sequence of
 * instructions to get there.
 * 
 * Example 1:
 * Input: 
 * target = 3
 * Output: 2
 * Explanation: 
 * The shortest instruction sequence is "AA".
 * Your position goes from 0->1->3.
 * 
 * Example 2:
 * Input: 
 * target = 6
 * Output: 5
 * Explanation: 
 * The shortest instruction sequence is "AAARA".
 * Your position goes from 0->1->3->7->7->6.
 * 
 * Note: 
 * 1 <= target <= 10000.
 */

class Solution {
public:
    int racecar(int target) {
        std::vector<int> steps_to_target(target + 3, 9090909);

        steps_to_target[0] = 0, steps_to_target[1] = 1; steps_to_target[2] = 4;
        for (int i = 3; i <= target; i++) {
            int k = 0;
            for (int j = i; j > 0; j /= 2) { k++; }

            if (i == (1 << k) - 1) {
                // i == 2^k - 1, the sum of 1 + 2 + 4 + ... + 2^(k-1)
                steps_to_target[i] = k;
            } else {
                for (int j = 0; j < k - 1; j++) {
                    // from ---> target
                    int from = i - (1 << (k - 1)) + (1 << j);
                    steps_to_target[i] = std::min(steps_to_target[i], steps_to_target[from] + k - 1 + j + 2);
                }
                // target <--- from
                int from = (1 << k) - 1 - i;
                steps_to_target[i] = std::min(steps_to_target[i], steps_to_target[from] + k + 1);
            }
        }

        return steps_to_target[target];
    }
};

