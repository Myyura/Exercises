...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        int sizeA = 0, sizeB = 0;
        for (ListNode *temp = headA; temp != NULL; temp = temp->next, sizeA++) {}
        for (ListNode *temp = headB; temp != NULL; temp = temp->next, sizeB++) {}
        
        ListNode *tempA = headA, *tempB = headB;
        for (;sizeA > sizeB; tempA = tempA->next, sizeA--);
        for (;sizeB > sizeA; tempB = tempB->next, sizeB--);
